# -*- coding: utf-8 -*-
"""
LSTM + GPT Sentiment Analysis Prediction Model
==============================================
Core Features:
1. Use price data, technical indicators and GPT sentiment analysis data
...
def load_sentiment_data() -> Dict[str, pd.DataFrame]:
    """Load sentiment analysis data from lstm_sentiment_series CSV files"""
    sentiment_data = {}
    
    # Look for lstm_sentiment_series files in root directory
    sentiment_files = []
...
def train_and_evaluate(data: pd.DataFrame, feature_cols: List[str], plot_title: str = "", multi_step: int = 1) -> Dict[str, Dict[str, float]]:
    ...
    if plot_title:
        print(f"\nGenerating prediction charts for {len(STOCK_LIST)} stocks...")
        ...
        for i, stock in enumerate(STOCK_LIST, 1):
            ...
            plt.title(f'{stock} - LSTM + GPT Sentiment Prediction Results')
            plt.xlabel('Date')
        ...
        plt.tight_layout()
        print("Displaying prediction charts...")
        plt.show()
...
def main():
    ...
    print("Loading GPT sentiment analysis data...")
    ...
    if sentiment_data:
        ...
        model_name = "LSTM + GPT Sentiment (with sentiment features)"
    else:
        print("No sentiment data found, using basic features")
